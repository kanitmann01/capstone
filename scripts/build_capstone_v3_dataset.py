"""Build capstone_v3 dataset combining fresh phishing + expanded legitimate sources.

Uses:
  - data/processed/tranco_legit.csv (2,159 URLs from previous fetch)
  - data/processed/phishing_fresh.csv (800 fresh URLs from feeds)

Stratified 70/15/15 split, host-disjoint, balanced classes.

Usage:
    python scripts/build_capstone_v3_dataset.py
"""
...
PHISHING_PATH = Path("data/processed/phishing_fresh.csv")
OUTPUT_DIR = Path("data/processed")
RANDOM_STATE = 42
MIN_LEGIT = 1500  # We want at least this many legitimate URLs
...
def build_manifest(
    ...
    return {
        "schema_version": "capstone_v3_manifest_v1",
...
def main() -> int:
    ...
    # Ensure we have enough legitimate URLs
    if len(legit_rows) < MIN_LEGIT:
        print(f"WARNING: Only {len(legit_rows)} legitimate URLs, need {MIN_LEGIT}")

    # Balance classes by downsampling majority
    target_phishing = len(phishing_rows)
    if len(legit_rows) > target_phishing:
        legit_rows = random.sample(legit_rows, target_phishing)
    ...
    if len(all_rows) < 1500:
        print(f"WARNING: Expected >= 1,500 rows, got {len(all_rows)}")
    ...
    assert total >= 1500, f"Total rows {total} < 1,500"
    ...
    write_csv(OUTPUT_DIR / "capstone_v3.csv", all_with_split)
    write_csv(OUTPUT_DIR / "capstone_v3_train.csv", train_rows)
    write_csv(OUTPUT_DIR / "capstone_v3_val.csv", val_rows)
    write_csv(OUTPUT_DIR / "capstone_v3_test.csv", test_rows)

    manifest = build_manifest(train_rows, val_rows, test_rows, all_rows)
    with (OUTPUT_DIR / "capstone_v3_manifest.json").open("w", encoding="utf-8") as f:
        json.dump(manifest, f, indent=2)

    print("Dataset build complete.")
    print(f"  capstone_v3.csv: {len(all_with_split)} rows")
    return 0
